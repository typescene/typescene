...
function writeContent(items, outDir, upName, upFileName) {
    return items.map(item => {
        ...
        if (item.summary) content += "<p><i>" + esc(item.summary) + "</i></p>";
        ...
        fs.writeFileSync(outDir + "/" + fileName,
            wrapItemContent(slug, content, itemName, upName, upFileName));
        return fileName;
    });
}
...
function wrapItemContent(slug, content, name, upName, upFileName) {
    return `<!DOCTYPE html>
<meta charset="utf-8">
<title>${esc(name)} - Typescene Documentation</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
