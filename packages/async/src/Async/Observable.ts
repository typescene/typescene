...
import { Signal, defineSignal, SignalConnection } from "./Signal";
...
export class ObservableValue<T> {
    ...
    public static fromSignal<T>(signal: Signal.Emittable<T, typeof Signal>) {
        var result = new ObservableValue<T>();
        result._ro = true;
        signal._connect(v => {
            result._ro = false;
    ...
    set value(value) {
        ...
        if (!this.shallow && (!this._setter || this._setting)) {
            ...
            if (value instanceof Object && value.constructor === Object) {
                value = <any>makeObjectObservable(value);
            }
        }
    ...
    private _addCheck(other: ObservableValue<any>) {
        ...
        (this._depChecks || (this._depChecks = [])).push((seen: {}) => {
    ...
    /** @internal True if value was set directly using .value */
    ...
    private _depChecks: ((seen: {}) => boolean)[];
...
export function unobserved(this: any, f) {
    var prevWatched = currentWatchedEvaling;
