import { defer, yieldAll } from "./Defer";
import { defineSignal, UnhandledException } from "./Signal";
...
export class Promise<T> implements PromiseLike<T> {
    ...
    private _resolve(value?: PromiseLike<T> | T, resolving?: {}) {
        if (!this._isResolved && !this._isRejected &&
            ...
            try {
                ...
                if (typeof thenFn === "function" &&
                    ...
                    thenFn.call(value,
                        v => { this._resolve(v, resolving) },
                        e => { this._reject(e, resolving) });
                }
                else {
    ...
    /** @internal Run a function that may resolve or reject the promise */
    ...
    private _reject(error: Error, resolving?: {}) {
        // set status and schedule handler chain (if any)
        if (!this._isResolved && !this._isRejected &&
            (!this._resolving || resolving === this._resolving)) {
    ...
    /* @internal */
    private _Resolve = defineSignal<T>();
    /* @internal */
    private _Reject = defineSignal<Error>();
}
