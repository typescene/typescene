...
/** List of functions to be run at animation frame intervals */
var _animDefer: Array<() => void> | undefined;
...
export class DOMUpdater {
    ...
    public updateAsync(content: Array<Node | RenderedOutput | Component | undefined>,
        ...
        var deferRender: (() => void) | undefined;
        ...
        for (var i = content.length - 1; i >= 0; i--) {
            ...
            if (item) {
                if (item instanceof Component) {
                    ...
                    components[i] = item;
                    seen[item.uid] = true;
                    let prev = deferRender;
                    let defer = this._renderComponent(item, nodes, i);
                    if (defer) {
                        deferRender = () => {
                            if (prev) Async.defer(prev);
                            defer!();
                        }
                    }
                }
        ...
        if (deferRender) Async.defer(deferRender);

        // prepare the actual DOM nodes update (scheduled using animation frames)
        if (!_animDefer) {
            // set up deferral callbacks first:
            _animDefer = [];
            let sched = () => {
                var a = _animDefer;
                _animDefer = undefined;
                a && a.forEach(d => Async.defer(d));
                Async.yieldAll();
            };
            if (typeof window.requestAnimationFrame === "function")
                window.requestAnimationFrame(sched);
            else
                window.setTimeout(sched, 15);
        }
    ...
    private _updateChildNodes(animatePositionMS?: number) {
        ...
        for (var i = 0; i < nNodes && current; i++) {
            ...
            if ((<HTMLElement>current).hasAttribute &&
                (<HTMLElement>current).hasAttribute(DOMUpdater.UNMANAGED_FLAG)) {
            ...
            (<HTMLElement>node).removeAttribute &&
                (<HTMLElement>node).removeAttribute(DOMUpdater.UNMANAGED_FLAG);
        ...
        if (Animation.isEnabled) {
            ...
            for (var c of this.components) {
                ...
                if (c && (!oldComponents ||
                    ...
                    if (nestedOut) {
                        if (nestedOut &&
                            (nestedOut["@context"] instanceof DOMUpdater) &&
                ...
                for (var i = 0, len = oldComponents.length; i < len; i++) {
                    ...
                    if (c && c.animations && c.animations.disappear) {
                        ...
                        if (removeElement && removeElement.nodeType === 1 &&
                            ...
                            elt.setAttribute(DOMUpdater.UNMANAGED_FLAG, "true");
                            this._nAnim++;
                            ...
                            playing.done.then(() => {
                                if (elt.parentNode === this.root &&
                                    elt.hasAttribute(DOMUpdater.UNMANAGED_FLAG)) {
                                    if (animatePositionMS! > 0)
