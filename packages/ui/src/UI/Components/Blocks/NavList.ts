...
import { _ComponentFactory, ComponentFactory,  UIValueOrAsync } from "../ComponentFactory";
...
export class NavList extends Block {
    ...
    protected beforeFirstRender(renderer: ComponentRenderer<this, any>) {
        ...
        renderer.watch(() => {
            ...
            if (container &&
                (<_ComponentFactory<any>>container).isComponentFactory) {
                // keep current container if possible, otherwise create
                container = ((<ComponentFactory<any>>container).uid ===
                    this._factoryUID) ? this._containerBlock.container :
                    Async.unobserved(() =>
                        (<ComponentFactory<any>>container).create());
                this._factoryUID = (<ComponentFactory<any>>container).uid;
            }
            else {
    ...
    public static withItems<T extends NavList>(
        this: { new (): T, with: typeof NavList.with },
        navItems: NavList.NavItem[]) {
        return this.with({ navItems });
    }
    ...
    public readonly NavChange = this.defineComponentSignal(NavList.SelectionSignal);
...
export class NavListSelectionHandler
    extends ComponentSignalHandler<NavList.SelectionEvent, typeof NavList.SelectionSignal> { }
