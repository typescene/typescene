...
export class List<BlockT extends Block> extends Block {
    ...
    public static withItems<T extends List<any>>(
        this: { new (): T, with: typeof List.with },
    ...
    public getChildren(): Component[] {
        ...
        var dividerFactory = this.divider && Divider.with(this.divider);
        this.items.forEach(item => {
            if (!item) return;

            // add a divider if needed (not before first item)
            if (dividerFactory && result.length) {
                var divider = dividers[item.uid] =
                    this._dividers[item.uid] ||
                    Async.unobserved(() => dividerFactory.create());
                result.push(divider);
            }
    ...
    public readonly SelectionChange = this.defineComponentSignal(List.SelectionSignal);
...
export class ListSelectionHandler<T extends Block>
    extends ComponentSignalHandler<List.ItemEvent<T>, typeof List.SelectionSignal> { }
