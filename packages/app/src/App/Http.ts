import { Promise, Signal, defineSignal } from "@typescene/async";

// declare fetch function if not defined
var _nativeFetch = (typeof (<any>window).fetch === "function") ?
    (<any>window).fetch : undefined;
var _canBlob = (() => {
    try {
...
export namespace Http {
    ...
    export var Requesting: Signal.Emittable<FetchOptions, typeof Signal> =
        defineSignal<FetchOptions>();
    ...
    export var Responded: Signal.Emittable<FetchResponse, typeof Signal> =
        defineSignal<FetchResponse>();
    ...
    export class MockResponse implements FetchResponse {
        ...
        private _readText(): PromiseLike<string> {
            try {
                ...
                if (this._init.responseText !== undefined)
    ...
    export function fetch(input: any, init?: any): any {
        ...
        if (_nativeFetch && !(<any>props).credentials &&
        ...
        return new Requesting(props).emit().then(results => {
            ...
            // start request
            return ((_nativeFetch && !config.FORCE_XHR_FETCH) ?
                _nativeFetch(props.url!, props) : _xhr(props))
                .then(response => (Responded(response), response));
        });
    }
    ...
    function _addQueryParams(url, params?: any) {
        if (params) {
