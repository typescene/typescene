...
import { ComponentFactory, UIValueOrAsync, makeFactory } from "./ComponentFactory";
import { ComponentRenderHandler, ComponentRenderer } from "./ComponentRenderer";
...
export abstract class Component extends Async.ObservableObject {
    ...
    public static addStyleOverride(style: Style) {
        var chain = Async.inject(this, {
            "@overrideStyle": (prev: Style): Style =>
    ...
    @Async.injectable
    public ["@createEventSignal"]<T>(
        id: string, signalClass: { new (data: T): any }, opt?: any):
        ComponentSignal.Emittable<T> {
        throw new TypeError();
    }
