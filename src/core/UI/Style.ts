...
export class Style {
    ...
    public hide(hidden?: boolean | Async.ObservableValue<boolean>): this;
    ...
    public select(selected?: boolean | Async.ObservableValue<boolean>): this;
        ...
        if (typeof obj === "string") {
            ...
            for (var prop in obj) {
                if (obj.hasObservableProperty(prop))
                    this._setProperty(prop, Async.observe(() => (<any>obj)[prop]));
            }
        }
        else {
    ...
    private _setProperty(name: string, value?: any) {
        if (!this._props) this._props = <any>new Async.ObservableObject();
    ...
    private _props?: Async.ObservableObject & { [id: string]: string | boolean | undefined };
